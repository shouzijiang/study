...
/**
 * 使用 submit-answer 接口校验并修正 all_answers.json 中答案。
...
$allLevelsPath = $levelDir . DIRECTORY_SEPARATOR . 'all_levels.json';
...
$sleepMsRetry = 5;   // 修正/重试时短暂等待，避免限流
...
if (is_file($allLevelsPath)) {
    ...
    if (is_array($allLevels)) {
        foreach ($allLevels as $item) {
            ...
            if ($lev >= $levelMin && $lev <= $levelMax && isset($item['data']) && is_array($item['data'])) {
                $levelDataCache[$lev] = $item['data'];
            }
        }
    }
}
// 未在 all_levels 中的关卡，尝试从单文件 1.json、2.json ... 补全
for ($i = $levelMin; $i <= $levelMax; $i++) {
    if (isset($levelDataCache[$i])) {
        continue;
    }
    $f = $levelDir . DIRECTORY_SEPARATOR . $i . '.json';
    if (is_file($f)) {
        $levelDataCache[$i] = json_decode(file_get_contents($f), true) ?: [];
    }
}
...
for ($level = $levelMin; $level <= $levelMax; $level++) {
    ...
    if ($levelData === null) {
        $failed[] = "level {$level}: 无关卡文件或 all_levels.json 中无该关卡";
        continue;
    }
    $wordArray = $levelData['wordArray'] ?? [];
    // 答案文件中无该关卡时，用关卡数据生成初始行（便于校验/修正后写入）
    if ($row === null) {
        $answerLength = (int)($levelData['answerLength'] ?? 0);
        $initialChars = $answerLength > 0 ? array_slice($wordArray, 0, $answerLength) : [];
        $row = [
            'level' => $level,
            'userAnswer' => implode('', $initialChars),
    ...
    // 从 wordArray 中取出可填错位的字（排除已正确位置占用的字），后续每次尝试都会用不同组合
    ...
    // 不再因排列数大而直接跳过，仍尝试最多 maxTriesPerLevel 次，可能在前若干次内命中正确答案

    // 用 wordArray 的可用字枚举错位排列，每次提交的 userAnswer 都不同
    $candidate = $arr;
    $tried = 0;
    $fixed = tryFixAnswer(
        $submitUrl,
...
file_put_contents($allAnswersPath, json_encode($out, $opts));

echo "\n完成：已更新 {$updated} 题，失败 " . count($failed) . " 题。\n";
if (!empty($failed)) {
    echo implode("\n", $failed) . "\n";
}
